...
const PromptBox = ({
  ...
  const icons = {
    ...
    "seedream":
      ...
      <svg role="img"
        ...
        viewBox="0 0 24 24"
        xmlns="http://www.w3.org/2000/svg"
        id="Bytedance--Streamline-Simple-Icons" height="24" width="24">
        <title>ByteDance</title>
        <path d="M19.8772 1.4685 24 2.5326v18.9426l-4.1228 1.0563V1.4685zm-13.3481 9.428 4.115 1.0641v8.9786l-4.115 1.0642v-11.107zM0 2.572l4.115 1.0642v16.7354L0 21.428V2.572zm17.4553 5.6205v11.107l-4.1228 -1.0642V9.2568l4.1228 -1.0642z"></path>
      </svg>
    ,
    "stable":
      <svg
  ...
  const modelList = [
    { label: "Google Nano Banana Pro", value: "google/imagen-4.0-ultra", credit: 6.3, icon: icons.google },
    { label: "Google Nano Banana Fast", value: "google/imagen-4.0-fast", credit: 2.0, icon: icons.google },
    { label: " Stable Diffusion XL Base 1.0 ", value: "stabilityai/stable-diffusion-xl-base-1.0", credit: 0.2, icon: icons.stable },
    { label: "Stable Diffusion 3 Medium", value: "stabilityai/stable-diffusion-3-medium", credit: 0.2, icon: icons.stable },
    { label: "Flux 2 Max", value: "black-forest-labs/FLUX.2-max", credit: 7.3, icon: icons.flux },
    { label: "Flux 1.1 Pro", value: "black-forest-labs/FLUX.1.1-pro", credit: 4.2, icon: icons.flux },
    { label: "Flux.1 Kontext-pro", value: "black-forest-labs/FLUX.1-kontext-pro", credit: 4.2, icon: icons.flux },
    { label: "Flux.1 Schnell", value: "black-forest-labs/FLUX.1-schnell", credit: 0.2, icon: icons.flux },
    { label: "Seedream 4.0", value: "ByteDance-Seed/Seedream-4.0", credit: 3.2, icon: icons.seedream },
    { label: "DALL-E 3", value: "dall-e-3", credit: 4.0, icon: icons.gpt },
    { label: "GPT Image 1.5", value: "gpt-image-1.5", credit: 1.6, icon: icons.gpt },
  ];
  ...
  const uploadImage = async (file) => {
    ...
    // console.log(data);
    const img_upload_res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/image`, { imageName: `${data.display_name}.jpg`, userId: id, imageUrl: data.secure_url, cloudinary_image_public_id: data.public_id, emailId: email, type: "JPG" });
  ...
  const GenerateImage = async () => {
    ...
    if (id) {
      ...
      try {
        ...
        console.log(error);
        setError(
          { "Status": "402", "Code": "Insufficient Fund", "Message": "Unable to Generate your Image", "error": error });
      }
    }
      ...
      >
          ...
          <div className="flex flex-wrap justify-center items-center">
...

            {
              ...
              <motion.div
                ...
                <span className="flex justify-center items-center w-full"> <img src={userCredits ? credit : creditLock} className="w-5 h-5 mr-2 inline-block" /> {userCredits ? `${(userCredits.remaining / 1000000).toFixed(1)} / ${(userCredits.monthUsageAllowance / 1000000).toFixed(1)}` : "Credits will be allocated after Initial Request from puter.js "}</span>
              </motion.div>
            }
          </div>
        ...
        <div
          ...
          <motion.button
            ...
            type="button"
            ref={menuRef}
            className={`mr-2 ${!isTab && "w-80"} relative border-[1.5px] border-red-500/35 hover:border-red-500/10  ${!button && "text-red-600/55 hover:bg-red-500/10"
              }
                ${button && "text-black/75 "}
                ${showMenu && " bg-red-100 text-black/55"
              } cursor-pointer duration-300  shrink-0 mt-3 gap-2 px-4 py-2.5 rounded-3xl flex items-center md:text-md text-md hover:bg-gray-300/35 focus:outline-none `}
            onClick={() => setShowMenu(!showMenu)} //setShowMenu((prev) => !prev)
          >
            <span className={`flex ${isTab ? "" : "px-2"} gap-2 w-full justify-between items-center text-red-500/53`}>{selectedModel.icon}{isTab ? "" : selectedModel.label}
              <FaChevronDown
          ...
          <motion.div
            ...
            onClick={() => {
              if (!loading && Prompt.trim() !== "") { GenerateImage(); }
              else {
                setButton(false);
                setLoading(false);
                setSubmit(false);
