  ...
  return (
    <div className="w-full h-full px-3 scroll-smooth">
        ...
        <motion.div initial={{ opacity: 0, y: 35 }}
          ...
          <div className="relative w-full h-full bg-white/88 rounded-[3.5em] md:rounded-[4.5em]  ">
            ...
            <div className=" relative w-full h-full py-10  rounded-[3.5em] md:rounded-[4.5em] bg-white flex md:flex-row flex-col justify-center items-center font-extrabold px-8 border-white border-2  ">
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .5, visualDuration: .5 }} className=" bg-linear-to-r md:text-4xl text-2xl from-red-500 via-pink-400 to-blue-400 bg-clip-text text-transparent pl-10 py-2" > Hi, I am Pranav and the Developer of the SAAI Image generator Website . For more Detsils about Me and My Projects , Take a look on this Page
              </motion.div>
            </div>
          </div>
        </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: isMobile ? 450 : 350 }} viewport={{ once: true }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: .8, visualDuration: 2.8, bounce: 0 }}
        className={` ${isTab ? " h-180 select-none flex-col mt-5 border-8 rounded-[3.5em] md:rounded-[4.5em] border-white " : ""} select-none w-full relative flex justify-center items-center h-135 `} >

        {isTab ?
          <div className="absolute  w-full h-full opacity-85 ai-aurora /bg-white rounded-[3.5em] md:rounded-[4.5em]"></div>
          :
          <img className="absolute mt-15  w-full h-full opacity-65 bg-linear-to-r to-red-500 via-yellow-400 from-violet-800 rounded-[3.5em] md:rounded-[4.5em] " src={isTab ? "" : pranav1.frame} />
        ...
        <motion.div initial={isTab ? { y: -70 } : { x: -20 }} viewport={{ once: true }} whileInView={isTab ? { y: 10 } : { x: 0 }} transition={{ duration: 0.6, visualDuration: 0.6, bounce: 0 }}
          ...
          className={`${isTab ?
            "left-30 mb-25 -translate-y-18 border-white /border-t-red-500 /border-l-yellow-300 /border-r-yellow-300 /border-b-red-500 "
            :
        ...
        <motion.div initial={isTab ? { y: -10 } : { x: 20 }} viewport={{ once: true }} whileInView={isTab ? { y: 0 } : { x: 0 }} transition={{ duration: 0.8, visualDuration: 0.8, bounce: 0 }}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`${isTab ? "font-extrabold text-4xl" : " text-5xl shrink-0 font-extrabold  "} flex justify-center  text-red-600 `}> Pranav N         </motion.div>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`${isTab ? "text-yellow-300/88 pt-5 text-2xl" : "text-3xl  shrink-0 ml-20 mt-4 mr-10  font-light "} font-extrabold flex justify-center text-yellow-300 `} >MERN Stack Developer        </motion.div>
        </motion.div>
      </motion.div>
      ...
      {project.map((key, index) => {
        return (
          <div key={index} className="w-full px-1 py-3">
            <motion.div initial={{ opacity: 0, y: 283 }} viewport={{ once: true }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, visualDuration: 0.4, bounce: 0 }} className="w-full flex md:flex-row flex-col px-8 backdrop-blur-lg  bg-white/10 rounded-4xl md:py-0 pt-5 pb-10 ">
              ...
              <div className="flex-1/2 w-full px-4 py-10 /text-shadow-red-500 /text-shadow-2xs flex-col flex justify-center items-center text-xl text-black/55">
                <motion.span initial={{ opacity: 0, x: 30 }} viewport={{ once: true }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 0.4, visualDuration: 0.4, bounce: 0 }}
                  className="md:text-3xl text-2xl pb-3.5 font-extrabold text-black/55">
                  {key.name}
                </motion.span>
